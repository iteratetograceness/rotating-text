...
| `--rt-tile-width`  | `0.62em`            | Minimum tile width.                                                                                                                                                             |
| `--rt-widest`      | `M` and `W`         | Letters every tile is at least as wide as (a CSS string, such as `'W'`), so new text never resizes the board. Set `none` to fit tiles to the text itself when it never changes. |
